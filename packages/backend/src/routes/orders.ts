...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Validate status against enum before hitting the DB
      if (status && !statusUpdateSchema.shape.status.safeParse(status).success) {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Build order items with computed subtotals and inject orderId in one pass
